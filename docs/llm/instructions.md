...
- This is a Bun project. Never use `npm`, `node`, or `typescript` commands.
- Always use `bun` commands. For example, use `bun install` to install dependencies, and `bun run <script>` to run scripts defined in `package.json`.
